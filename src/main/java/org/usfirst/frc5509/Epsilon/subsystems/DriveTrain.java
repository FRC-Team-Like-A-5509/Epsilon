...
public class DriveTrain extends Subsystem {
    ...
    public DriveTrain() {
        ...
        backRightDrive = new WPI_TalonSRX(2);
        ...
        frontLeftDrive = new WPI_TalonSRX(4);
        
        
        
        frontRightSwerve = new WPI_TalonSRX(5);
        
        
        
        backRightSwerve = new WPI_TalonSRX(6);
        
        
        
        backLeftSwerve = new WPI_TalonSRX(7);
        
        
        
        frontLeftSwerve = new WPI_TalonSRX(8);
	...
	public void drive(double rotation, double x, double y) {
		...
		if (Math.abs(frontRightSwerve.getClosedLoopError(0)) < 20) {
			frontRightDrive.set(speeds[0] * MAX_SPEED);
		}
		if (Math.abs(frontLeftSwerve.getClosedLoopError(0)) < 20) {
			frontLeftDrive.set(speeds[1] * MAX_SPEED);
		}
		if (Math.abs(backLeftSwerve.getClosedLoopError(0)) < 20) {
			backLeftDrive.set(speeds[2] * MAX_SPEED);
		}
		if (Math.abs(backRightSwerve.getClosedLoopError(0)) < 20) {
			backRightDrive.set(speeds[3] * MAX_SPEED);
		}

		frontRightSwerve.set(ControlMode.Position, ticks[0] / 3);
		frontLeftSwerve.set(ControlMode.Position, ticks[1]);
		backLeftSwerve.set(ControlMode.Position, ticks[2]);
		backRightSwerve.set(ControlMode.Position, ticks[3] / 3);
